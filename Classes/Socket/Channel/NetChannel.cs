...
public class NetChannel
{
    ...
    protected SocketBase m_net_socket = null;
    protected ByteArray m_by_buffer = null;
    //由于收到数据时，已经通过lock全局锁同步到主线程，所以可以共用一个buffer
    static protected ByteArray m_dispatcher_buffer = new ByteArray(NetID.SendRecvMaxSize, NetID.SendRecvMaxSize);
    ...
    public virtual void Setup(SocketBase socket, long conn_idx)
    {
        m_net_socket = socket;
        m_conn_idx = conn_idx;
    }
        ...
        while (m_by_buffer.Available >= NetID.PacketHeadSize)
        {
            if (m_by_buffer.Peek(ref head_by, NetID.PacketHeadSize))
            {
                ...
                if (m_by_buffer.Available >= msg_length + NetID.PacketHeadSize)
                {
                    ...
                    {
                        m_net_socket.HandleReceive(m_conn_idx, header, m_dispatcher_buffer);
                    }
                }
                else
                    break;
